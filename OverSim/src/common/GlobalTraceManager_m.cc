...
GlobalTraceManagerMessage::GlobalTraceManagerMessage(const GlobalTraceManagerMessage& other) : cPacket(other)
{
    copy(other);
}
...
void GlobalTraceManagerMessage::copy(const GlobalTraceManagerMessage& other)
{
...
void GlobalTraceManagerMessage::setInternalNodeId(int internalNodeId)
{
    this->internalNodeId_var = internalNodeId;
}
...
void GlobalTraceManagerMessage::setLineNumber(int lineNumber)
{
    this->lineNumber_var = lineNumber;
}
...
int GlobalTraceManagerMessageDescriptor::findField(void *object, const char *fieldName) const
{
    cClassDescriptor *basedesc = getBaseClassDescriptor();
    int base = basedesc ? basedesc->getFieldCount(object) : 0;
    if (fieldName[0]=='i' && strcmp(fieldName, "internalNodeId")==0) return base+0;
    if (fieldName[0]=='l' && strcmp(fieldName, "lineNumber")==0) return base+1;
...
std::string GlobalTraceManagerMessageDescriptor::getFieldAsString(void *object, int field, int i) const
{
    cClassDescriptor *basedesc = getBaseClassDescriptor();
    if (basedesc) {
        if (field < basedesc->getFieldCount(object))
    ...
    GlobalTraceManagerMessage *pp = (GlobalTraceManagerMessage *)object; (void)pp;
    switch (field) {
        case 0: return long2string(pp->getInternalNodeId());
        case 1: return long2string(pp->getLineNumber());
        default: return "";
    }
}
